...
-- Stable helper function: returns the business_id of the currently authenticated user.
-- SECURITY DEFINER + STABLE allows Postgres to cache the result within a query,
-- avoiding N repeated subqueries in RLS policy evaluation.
...
CREATE POLICY "purchase_order_items_business_access" ON purchase_order_items
  FOR ALL USING (
    po_id IN (
      SELECT id FROM purchase_orders WHERE business_id = current_business_id()
    )
  );
